...
def findElementByXPathClear(xpath):
    inputXPath1 = driver.find_element_by_xpath(xpath)
    inputXPath1.clear()

def  findItemInputViewt(xpath,text):
    inputXPath2 = driver.find_element_by_xpath(xpath)
    inputXPath2.send_keys(text)
    time.sleep(1)

def clickDelay(xpath):
    clickXPath = driver.find_element_by_xpath(xpath)
    driver.execute_script("arguments[0].click();",clickXPath)
    time.sleep(1)

def listElementXPathAndClass(xpath,classname):
    list = driver.find_elements_by_xpath(xpath)
    arrayList = list.__getitem__(0).find_elements_by_class_name(classname)
    return  arrayList

def codeVerification(img):
